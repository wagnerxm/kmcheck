...
// cacheadas na primeira carga. data/today.json (dados do pipeline) também é rede-primeiro
// para sempre pegar os picks mais recentes.
self.addEventListener('fetch', e => {
  ...
  // data/today.json do pipeline: rede primeiro (mesma estratégia do documento)
  const isData = url.pathname.endsWith('/data/today.json');
  // Google Fonts (CSS e woff2): cachear na primeira carga, depois servir do cache
  const isFont = url.hostname === 'fonts.googleapis.com' || url.hostname === 'fonts.gstatic.com';
  if (isDoc || isData) {
    // Rede primeiro para o documento e dados do pipeline — garante que atualizações cheguem rápido
    e.respondWith(
      fetch(e.request).then(resp => {
        const copy = resp.clone();
        caches.open(CACHE).then(c => c.put(e.request, copy));
        return resp;
      }).catch(() => caches.match(e.request, {ignoreSearch: true}).then(r => r || (isData ? new Response('{}', {status: 404}) : caches.match('./'))))
    );
    return;
  }
  if (isFont) {
